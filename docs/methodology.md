...
V4 factor analysis showed nursing home capacity loading 0.91 on its own isolated factor (Factor 3), completely independent of combined care nonprofit density (F1: 0.51) and FQHC density (F1: 0.59). This confirms nursing home infrastructure is a genuinely distinct dimension of elder care. The V6 weight (35% of Pillar 2, within a three-metric structure of FQHC 45%, nursing homes 35%, child care 20%) reflects this distinctness while acknowledging that nursing homes serve a narrower population (elderly specifically) than FQHCs (full community age distribution).
...
**Measurement note**: CBP counts establishments, not licensed capacity (slots). Two cities with the same establishment count may have materially different total slot counts depending on facility size. This is a supply-presence metric. Both the numerator (CBP establishments) and denominator (ACS B01001 under-5 population) are county-level, using the county FIPS codes assigned to each city in config.py. This ensures numerator and denominator share the same geographic footprint; the metric reflects county-wide child care density rather than city-specific density. See limitation 4b in Section 9.
...
12. **Healthcare coverage metric ceiling effect**: 31 of 69 cities (45%) score exactly 100.0 on this metric. The cause is a denominator mismatch: the numerator (C27007) counts all Medicaid and CHIP enrollees, including CHIP-covered children whose household incomes fall between 150% and 300% FPL — above the eligibility ceiling used in the denominator (C17002, 0–149% FPL). In high-expansion states with strong CHIP enrollment, the enrollee count can therefore exceed the estimated eligible-population count, producing a raw coverage rate above 100% that is then capped to 100 by the scoring formula. The result is that the metric functions as a near-binary indicator (Medicaid expansion vs. non-expansion) rather than a continuous measure within the expansion-state group. Researchers comparing two expansion-state cities on healthcare coverage should note that differences at the ceiling carry no signal. A future version may address this by broadening the denominator to include CHIP-eligible income bands (0–300% FPL) or by using an alternative data source that avoids the mismatch.

13. **ACS 5-year estimate smoothing**: The 2022 ACS 5-year estimates pool survey responses from 2018–2022, smoothing rapid demographic shifts. Cities undergoing rapid population change (fast-growing Sun Belt metros, post-pandemic migration destinations) may have metrics that lag current conditions by 1–3 years. Close rankings — cities within 3–4 points of each other — should not be over-read; differences of that size can fall within ACS estimation variance, particularly for smaller cities where the survey sample is thinner.

14. **Total-population vs. need-adjusted denominator**: All per-capita density metrics use total city population as the denominator. An alternative framing normalizes by the population most likely to need care-related services — residents below 150% of the Federal Poverty Level. These two denominators tell different stories: the total-population denominator measures citywide supply per resident, while a need-adjusted denominator measures supply relative to likely demand. Large cities with concentrated poverty (where the total population is high but the low-income population represents a smaller share) tend to score lower under total-population normalization than under need-adjusted normalization. V5 reports care nonprofit density per 10,000 residents at 0–150% FPL as a shadow diagnostic to test whether rankings diverge materially between the two framings.
