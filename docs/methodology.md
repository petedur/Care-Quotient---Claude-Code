...
12. **Healthcare coverage metric ceiling effect**: The healthcare coverage benchmark is 100% — theoretically correct, since full enrollment among eligible residents is the policy goal. In practice, 31 of 69 cities (45%) score exactly 100.0, because Medicaid-expansion states with effective outreach reach near-complete reported enrollment among the 0–149% FPL population in ACS survey data. This means the metric functions as a near-binary indicator between expansion and non-expansion states rather than a continuous measure. Discrimination *within* expansion states is limited; differences between expansion-state cities at the ceiling carry no signal. Researchers comparing two expansion-state cities should weight Pillar 3 scores accordingly. A future version may revise this benchmark to improve within-group discrimination.
